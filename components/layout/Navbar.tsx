...
export default function Navbar() {
  ...
  return (
    <>
      {/* 
        CHANGED TO ABSOLUTE: 
        The navbar now stays perfectly at the top of the document 
        and will not follow the user down the screen!
      */}
      <header className="absolute top-6 left-0 right-0 z-50 mx-auto w-[95%] max-w-[1400px]">
        <div className="w-full rounded-full flex items-center justify-between px-6 md:px-10 py-3 bg-[#050505]/60 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.3)] relative z-50">
          ...
          {/* ALL LINKS UPDATED TO UPPERCASE MONTSERRAT FOR PREMIUM BRANDING */}
          <nav className="hidden lg:flex items-center gap-10 text-[12px] md:text-xs text-white font-bold tracking-[0.2em] uppercase drop-shadow-md" style={{ fontFamily: "'Montserrat', sans-serif" }}>
            <Link href="/#locations" className="relative group px-1 py-1 overflow-hidden flex flex-col items-center">
              ...
              <span className="absolute bottom-0 w-0 h-[1px] bg-[#C7A26A] transition-all duration-500 ease-out group-hover:w-full"></span>
            </Link>
          ...
          <div className="hidden lg:flex items-center">
            <Link href="/#contact" className="px-6 py-2.5 rounded-full text-[10px] md:text-xs tracking-[0.2em] uppercase transition-all duration-300 border border-[#C7A26A]/50 bg-transparent text-white hover:bg-[#C7A26A] hover:text-black hover:border-[#C7A26A] shrink-0 font-bold" style={{ fontFamily: "'Montserrat', sans-serif" }}>
              Contact Us
            </Link>
          </div>
      ...
      {/* MOBILE MENU UPDATED TO MONTSERRAT */}
      <div className={`fixed inset-0 z-40 bg-[#050505]/95 backdrop-blur-3xl flex flex-col items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMobileMenuOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full pointer-events-none"}`}>
        <nav className="flex flex-col items-center gap-8 text-center">
          <Link href="/" className="text-2xl font-bold tracking-[0.2em] uppercase text-white hover:text-[#C7A26A] transition-colors" style={{ fontFamily: "'Montserrat', sans-serif" }} onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
          <Link href="/#locations" className="text-2xl font-bold tracking-[0.2em] uppercase text-white hover:text-[#C7A26A] transition-colors" style={{ fontFamily: "'Montserrat', sans-serif" }} onClick={() => setIsMobileMenuOpen(false)}>Locations</Link>
          <Link href="/about" className="text-2xl font-bold tracking-[0.2em] uppercase text-white hover:text-[#C7A26A] transition-colors" style={{ fontFamily: "'Montserrat', sans-serif" }} onClick={() => setIsMobileMenuOpen(false)}>About Us</Link>
