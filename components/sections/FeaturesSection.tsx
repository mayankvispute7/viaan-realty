...
const features = [
  ...
  {
    icon: Scale,
    title: "Strong Negotiations",
    desc: "Our market authority allows us to negotiate unparalleled deals, securing your dream asset at the absolute best price point."
  },
  {
    icon: Gem,
...
export default function FeaturesSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 3D Fold-In Animation for the cards
      gsap.fromTo(
        cardsRef.current?.children ? Array.from(cardsRef.current.children) : [],
        { 
          opacity: 0, 
          y: 80, 
          rotationX: -45, // Starts tilted backwards in 3D space
          transformPerspective: 1000 
        },
        {
          opacity: 1,
          y: 0,
          rotationX: 0, // Folds flat into place
          duration: 1.2,
          stagger: 0.2,
          ease: "power3.out",
          scrollTrigger: {
  ...
  return (
    <section ref={sectionRef} id="features" className="bg-[#F7F3EE] py-24 md:py-32 px-6 md:px-12 relative overflow-hidden">
      ...
      {/* NEW: CSS for the continuous 3D loop and hover speed-up */}
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .group:hover .icon-3d-loop {
          animation-duration: 1.5s; /* Speeds up dramatically on hover for a wow effect! */
        }
      `}} />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* 4-Grid Features with 3D Hover Effects */}
        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index}
              className="group relative bg-white p-8 rounded-2xl border border-black/5 transition-all duration-500 ease-out hover:-translate-y-4 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)]"
              style={{ transformStyle: "preserve-3d" }}
            >
              {/* Animated Icon Container (overflow-visible added so 3D rotation isn't clipped) */}
              <div className="w-16 h-16 rounded-full bg-[#F7F3EE] border border-[#C7A26A]/30 flex items-center justify-center mb-6 relative overflow-visible group-hover:border-[#C7A26A] transition-colors duration-500">
                ...
                {/* NEW: The 3D Looping Icon Wrapper */}
